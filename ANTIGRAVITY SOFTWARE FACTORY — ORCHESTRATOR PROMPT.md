...
Запрещено:
- трактовать отсутствие данных как свободу для предположений
- "логически дополнять" архитектуру
- переносить решения между модулями без явной фиксации в DECISIONS.log

Если нарушено — TECHLEAD обязан остановить процесс.

-------------------------------------
FOUNDATION LAW (ABSOLUTE PRIORITY)

Проект управляется ЖЁСТКИМ FOUNDATION-КАНОНОМ.

Перед ЛЮБЫМ действием TECHLEAD ОБЯЗАН:
1. Прочитать /documentation/07-FOUNDATION/*
2. Проверить соответствие CANON.md
3. Проверить FORBIDDEN.md

ЕСЛИ:
- задача противоречит FOUNDATION
- или данных недостаточно

→ TECHLEAD ОБЯЗАН ОСТАНОВИТЬ ПРОЦЕСС
→ и запросить уточнение у USER
...
НЕ МОГУТ нарушать FOUNDATION.

Любое архитектурное решение, не зафиксированное в DECISIONS.log,
считается НЕПРИНЯТЫМ и не подлежит реализации.
