    ...
    return (
        <div className="space-y-4">
            ...
            <div className={`p-4 rounded border ${summary.canCommit ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                <h4 className={`text-lg font-medium ${summary.canCommit ? 'text-green-800' : 'text-red-800'}`}>
                    {summary.canCommit ? 'Operation Allowed' : 'Operation Blocked'}
                </h4>
                <div className="flex gap-4 mt-2 text-sm">
                    <span className="font-medium text-red-600">Блокирующий: {summary.totalBlocking}</span>
                    <span className="font-medium text-yellow-600">Warnings: {summary.totalWarning}</span>
            ...
            {summary.totalBlocking > 0 && (
                <div className="mt-4">
                    <h5 className="font-medium text-red-700 mb-2">Blocking Issues (ALL-OR-NOTHING: Fix these to proceed)</h5>
