...
const LifecyclePanel: React.FC<LifecyclePanelProps> = ({ status, onActivate, onArchive, isLoading }) => {
    return (
        <div className="bg-white border border-black/10 rounded-lg p-4">
            <h3 className="text-xs font-medium text-[#717182] uppercase tracking-widest mb-4">
                {UI_TEXT.TAB_LIFECYCLE}
            </h3>

            <div className="flex items-center justify-between mb-6 p-3 bg-[#F3F3F5] rounded border border-black/10">
                <span className="text-sm font-medium text-[#717182]">{UI_TEXT.COL_STATUS}</span>
            ...
            <div className="space-y-3">
                ...
                {status === 'draft' && (
                    ...
                    >
                        ...
                        <span className="font-medium text-xs uppercase">{UI_TEXT.BTN_ACTIVATE}</span>
                    </button>
                )}
                ...
                {status === 'active' && (
                    ...
                    >
                        ...
                        <span className="font-medium text-xs uppercase">{UI_TEXT.BTN_ARCHIVE}</span>
                    </button>
                )}

                {/* ARCHIVED STATE */}
                {status === 'archived' && (
                    <div className="text-center py-2 text-xs text-[#717182] font-mono">
                        {UI_TEXT.TERM_STATE_NOTE}
                    </div>
                )}
            </div>

            <div className="mt-4 pt-4 border-t border-black/10 text-[10px] text-slate-600 leading-relaxed">
                <p>
