...
import { useCreateDepartmentMutation, useGetDepartmentsQuery } from '../api/ofsApi';
...
export default function CreateDepartmentModal({ onClose, onSuccess }: Props) {
  const [name, setName] = useState('');
  const [description, setDescription] = useState('');
  const [parentId, setParentId] = useState('');
  const [hierarchyLevel, setHierarchyLevel] = useState<number>(4);
  const [departmentType, setDepartmentType] = useState<'operational' | 'support' | 'management'>('operational');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const functionsArray = functions.trim() 
        ? functions.split(',').map(f => f.trim()).filter(Boolean)
        : undefined;
      ...
      setError(err.data?.error?.message || 'Ошибка создания департамента');
    }
  };
      ...
      <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-semibold">Создать департамент</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Департамент построения организации"
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Краткое описание департамента и его функций"
              rows={3}
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            />
          </div>
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-gray-500">
              Из Конституции Photomatrix - 7-уровневая иерархия
            </p>
          </div>
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-gray-500">
              Оставьте пустым для департамента верхнего уровня
            </p>
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="HR-аналитика, Скрининг кандидатов, Онбординг через бот"
              rows={2}
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            />
            <p className="mt-1 text-xs text-gray-500">
              Укажите функции департамента, разделяя их запятыми
            </p>
          </div>
