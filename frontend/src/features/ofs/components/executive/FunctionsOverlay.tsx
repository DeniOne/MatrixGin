...
export default function FunctionsOverlay() {
    ...
    return (
        <div className="absolute left-6 bottom-32 w-80 bg-gray-900/90 backdrop-blur-xl border border-gray-800 rounded-2xl p-5 shadow-2xl z-20 animate-in fade-in slide-in-from-left-4">
            <div className="flex items-center gap-3 mb-6">
                <div className="p-2 bg-purple-500/20 rounded-lg">
                    <Activity className="w-5 h-5 text-purple-400" />
                </div>
                <div>
                    <h3 className="text-sm font-bold text-white uppercase tracking-tight">Функциональная Карта</h3>
                    <p className="text-[10px] text-gray-500 uppercase font-bold tracking-widest mt-0.5">Покрытие и Нагрузка</p>
                </div>
            </div>
                ...
                {functionData.map((fn, i) => (
                    <div key={i} className="space-y-1.5">
                        <div className="flex justify-between items-center text-[11px]">
                            ...
                            <span className={`text-[9px] px-1 rounded-sm font-bold uppercase ${fn.status === 'stable' ? 'text-green-400' :
                        ...
                        <div className="flex justify-between text-[8px] uppercase tracking-tighter font-bold">
            ...
            <div className="mt-6 pt-4 border-t border-gray-800">
                <div className="flex items-center justify-between text-[10px] text-gray-400 font-bold uppercase">
                    ...
                    <span className="text-white">66.2%</span>
                </div>
            </div>
        </div>
    );
