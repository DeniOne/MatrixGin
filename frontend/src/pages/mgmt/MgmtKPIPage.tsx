    ...
    return (
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="flex justify-between items-center mb-8">
                <h1 className="text-3xl font-medium text-gray-900">Студия KPI и OKR</h1>
                    ...
                    <div key={obj.id} className="bg-white rounded-xl border p-6">
                        <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center">
                                ...
                                <h3 className="font-medium text-lg text-gray-900">{obj.title}</h3>
                            </div>
                            <span className="text-sm font-medium text-indigo-600">{obj.progress}%</span>
                        </div>
                            ...
                            {obj.keyResults.map((kr, idx) => (
                                <div key={idx} className="flex items-center text-sm">
                                    <div className={`w-5 h-5 rounded border-2 mr-3 flex items-center justify-center ${kr.done ? 'bg-green-500 border-green-500' : 'border-gray-300'
                                        ...
                                        {kr.done && <span className="text-[#030213] text-xs">✓</span>}
                                    </div>
                                    <span className={kr.done ? 'text-[#717182] line-through' : 'text-gray-700'}>{kr.title}</span>
                                </div>
                            ))}
                        </div>
                    </div>
