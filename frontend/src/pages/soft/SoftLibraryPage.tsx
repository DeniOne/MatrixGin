    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
                <div>
                    <h1 className="text-3xl font-medium text-gray-900">Библиотека знаний</h1>
                    <p className="text-[#717182] mt-1">Книги, подкасты и статьи для вашего развития</p>
                </div>
            ...
            <div className="flex space-x-1 bg-gray-100 p-1 rounded-xl mb-8 w-fit">
                <button
                    ...
                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'all' ? 'bg-white text-gray-900 shadow-sm' : 'text-[#717182] hover:text-gray-700'}`}
                >
                    Все
                </button>
                <button
                    onClick={() => setActiveTab('book')}
                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center ${activeTab === 'book' ? 'bg-white text-gray-900 shadow-sm' : 'text-[#717182] hover:text-gray-700'}`}
                >
                    <Book className="w-4 h-4 mr-2" /> Книги
                </button>
                <button
                    onClick={() => setActiveTab('podcast')}
                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center ${activeTab === 'podcast' ? 'bg-white text-gray-900 shadow-sm' : 'text-[#717182] hover:text-gray-700'}`}
                >
                    <Headphones className="w-4 h-4 mr-2" /> Подкасты
                </button>
                <button
                    onClick={() => setActiveTab('article')}
                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center ${activeTab === 'article' ? 'bg-white text-gray-900 shadow-sm' : 'text-[#717182] hover:text-gray-700'}`}
                >
                ...
                {filteredResources.map(resource => (
                    <div key={resource.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-lg transition-all group flex flex-col h-full">
                        <div className="h-48 overflow-hidden relative">
                            ...
                            {resource.isNew && (
                                <div className="absolute top-3 left-3 bg-indigo-600 text-[#030213] text-xs font-medium px-2 py-1 rounded">
                                    NEW
                                </div>
                            )}
                        ...
                        <div className="p-5 flex-1 flex flex-col">
                            <div className="flex items-center text-xs text-[#717182] mb-2 space-x-2">
                            ...
                            <h3 className="font-medium text-gray-900 mb-1 line-clamp-2">{resource.title}</h3>
                            <p className="text-sm text-[#717182] mb-3">{resource.author}</p>

                            <div className="mt-auto pt-3 border-t border-gray-100 flex items-center justify-between text-xs text-[#717182]">
                                <span className="flex items-center">
                                ...
                                <div className="flex space-x-1">
                                    {resource.tags.slice(0, 2).map(tag => (
                                        <span key={tag} className="bg-gray-50 px-1.5 py-0.5 rounded text-[#717182]">
                                            {tag}
                                        </span>
                                    ))}
                                </div>
