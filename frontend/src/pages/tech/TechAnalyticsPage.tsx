...
const TechAnalyticsPage: React.FC = () => {
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <h1 className="text-3xl font-medium text-gray-900 mb-8">Аналитика Активов</h1>
            ...
            <div className="bg-white rounded-xl border p-6">
                <h2 className="font-medium text-gray-900 mb-4">Состояние оборудования</h2>
                <div className="space-y-4">
                    {equipment.map((item, idx) => (
                        <div key={idx} className="flex items-center justify-between py-3 border-b last:border-0">
                            <div>
                                <h3 className="font-medium text-gray-900">{item.name}</h3>
                                <p className="text-xs text-[#717182]">Последнее ТО: {item.lastMaintenance}</p>
                            </div>
                            <div className="text-right">
                                <div className="text-sm font-medium text-gray-900">{item.uptime}%</div>
                                <div className="text-xs text-[#717182]">uptime</div>
                            </div>
                        </div>
                    ))}
                </div>
