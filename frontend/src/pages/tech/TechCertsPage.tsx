...
const TechCertsPage: React.FC = () => {
    ...
    return (
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <h1 className="text-3xl font-medium text-gray-900 mb-8">Сертификация</h1>
                    ...
                    <div key={cert.id} className="bg-white rounded-xl border p-6">
                            ...
                            <div className="flex items-center">
                                <div className={`p-3 rounded-lg mr-4 ${cert.status === 'earned' ? 'bg-green-100' :
                                    ...
                                    {cert.status === 'earned' ? <Award className="w-6 h-6 text-green-600" /> :
                                        cert.status === 'locked' ? <Lock className="w-6 h-6 text-[#717182]" /> :
                        ...
                        {cert.status === 'in-progress' && cert.progress && (
                            <div>
                                ...
                                <button className="mt-3 w-full py-2 bg-indigo-600 text-[#030213] rounded-lg text-sm font-medium hover:bg-indigo-700">
                                    Продолжить обучение
                                </button>
                            </div>
                        )}
                        ...
                        {cert.status === 'locked' && (
                            <p className="text-sm text-[#717182]">Требуется завершить предыдущие сертификации</p>
                        )}
                    </div>
                ))}
            </div>
