...
const TechLabsPage: React.FC = () => {
    ...
    return (
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Песочница и Лаборатория</h1>
            <p className="text-gray-500 mb-8">Тестируйте новые технологии и функции в безопасной среде</p>
                ...
                {labs.map(lab => (
                    <div key={lab.id} className={`bg-white rounded-xl border p-6 ${lab.access ? 'hover:shadow-md transition-shadow' : 'opacity-70'}`}>
                        <div className="flex items-center justify-between mb-4">
                            ...
                            {!lab.access && <Lock className="w-5 h-5 text-gray-400" />}
                        </div>
                        <h3 className="font-bold text-lg text-gray-900 mb-2">{lab.title}</h3>
