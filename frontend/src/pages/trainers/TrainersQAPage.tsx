...
const TrainersQAPage: React.FC = () => {
    ...
    return (
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <h1 className="text-3xl font-bold text-gray-900 mb-8">Контроль Качества</h1>

            <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-6 mb-8 border border-green-200">
                <div className="flex items-center justify-between">
                    <div>
                        <h3 className="text-sm font-medium text-gray-500 mb-1">Средний балл качества</h3>
            ...
            <h2 className="text-xl font-bold text-gray-900 mb-4">История аудитов</h2>
            <div className="space-y-3">
                {audits.map(audit => (
                    <div key={audit.id} className="bg-white p-5 rounded-xl border">
                        <div className="flex items-center justify-between mb-2">
                            <h3 className="font-bold text-gray-900">{audit.session}</h3>
                            <div className="flex items-center">
                                <span className="text-lg font-bold text-gray-900 mr-1">{audit.score}</span>
                                <Star className="w-5 h-5 text-yellow-500 fill-current" />
                            </div>
                        </div>
                        <div className="flex items-center justify-between">
                            <p className="text-sm text-gray-500">Аудитор: {audit.auditor}</p>
