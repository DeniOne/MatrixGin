...
const PhotoTrainerDirectoryPage: React.FC = () => {
  ...
  if (isLoading) return <div className="text-[#030213]">Загрузка наставников…</div>;
  ...
  return (
    <div className="max-w-7xl mx-auto text-[#030213]">
      <h1 className="text-2xl font-medium mb-4">Наставники — Фотомастерство</h1>

      {trainers.length === 0 ? (
        <div className="text-[#717182]">Пока нет наставников.</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {trainers.map((t) => (
            <div key={t.id} className="bg-white border border-black/10 rounded-lg p-4">
              <div className="flex items-center justify-between mb-2">
                <div className="font-medium">{t.userName || t.id}</div>
