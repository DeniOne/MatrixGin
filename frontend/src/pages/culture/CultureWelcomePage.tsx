        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="text-center mb-12">
                <div className="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-br from-purple-500 to-indigo-600 rounded-full mb-4 shadow-lg">
                    <Sparkles className="w-10 h-10 text-[#030213]" />
                </div>
                <h1 className="text-4xl font-medium text-gray-900 mb-3">Welcome-трек</h1>
            ...
            <div className="mb-10">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="text-sm font-medium text-indigo-600">{Math.round((currentStep / steps.length) * 100)}%</span>
                </div>
                ...
                {steps.map((step) => (
                    ...
                    >
                        <div className="flex items-center justify-between mb-2">
                            <div className={`p-2 rounded-lg ${step.id === currentStep ? 'bg-indigo-600 text-[#030213]' :
                                    step.completed ? 'bg-green-500 text-[#030213]' : 'bg-gray-100 text-gray-600'
                                }`}>
                                {step.completed ? <CheckCircle className="w-5 h-5" /> : step.icon}
                            </div>
                            <span className="text-xs font-medium text-[#717182]">Шаг {step.id}</span>
                        </div>
                        <h3 className="font-medium text-sm text-gray-900 mb-1">{step.title}</h3>
                        <p className="text-xs text-[#717182] line-clamp-2">{step.description}</p>
                    </button>
                ))}
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
                <div className="bg-gradient-to-r from-indigo-600 to-purple-600 px-8 py-6 text-[#030213]">
                    <h2 className="text-2xl font-medium mb-2">{currentStepData.title}</h2>
                ...
                <div className="p-8">
                    {currentStepData.content.type === 'video' && (
                        <div className="space-y-6">
                            <div className="aspect-video bg-white rounded-xl overflow-hidden relative group cursor-pointer">
                                <img
                                ...
                                <div className="absolute bottom-4 left-4 bg-white/70 text-[#030213] px-3 py-1 rounded text-sm font-medium">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {currentStepData.content.data.values.map((value: any, idx: number) => (
                                <div key={idx} className="bg-gradient-to-br from-indigo-50 to-purple-50 p-6 rounded-xl border border-indigo-100">
                                    <h3 className="font-medium text-lg text-indigo-900 mb-2">{value.name}</h3>
                            ...
                            {currentStepData.content.data.sections.map((section: string, idx: number) => (
                                <div key={idx} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200 hover:border-indigo-300 hover:bg-indigo-50 transition-all cursor-pointer group">
                                    <div className="flex items-center">
                                        <FileText className="w-5 h-5 text-[#717182] mr-3 group-hover:text-indigo-600" />
                                        <span className="font-medium text-gray-900">{section}</span>
                                    </div>
                                    <ArrowRight className="w-5 h-5 text-[#717182] group-hover:text-indigo-600" />
                                </div>
                            ))}
                        </div>
                    )}
                    ...
                    {currentStepData.content.type === 'quiz' && (
                        <div className="text-center py-12">
                            ...
                            <h3 className="text-xl font-medium text-gray-900 mb-2">Завершающий тест</h3>
                            <p className="text-gray-600 mb-6">
                                Проверьте свои знания и получите сертификат о прохождении онбординга
                            </p>
                            <button className="px-6 py-3 bg-indigo-600 text-[#030213] rounded-lg font-medium hover:bg-indigo-700 transition-colors">
                ...
                <div className="px-8 py-6 bg-gray-50 border-t border-gray-200 flex justify-between items-center">
                    ...
                    <button
                        ...
                        className={`px-6 py-2 bg-indigo-600 text-[#030213] rounded-lg font-medium hover:bg-indigo-700 transition-colors flex items-center ${currentStep === steps.length ? 'opacity-50 cursor-not-allowed' : ''
                            }`}
